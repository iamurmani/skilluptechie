...
const AdminSideBar = () => {
  ...
  return (
    <div className="bg-[#121621] w-[110px] md:w-[317px] h-screen overflow-hidden overflow-y-scroll">
      ...
      <div className="ml-3">
        <h4 className="w-24 h-6 font-bold md:text-xl text-[#8A94A6] font-Inria">
          Dashboard
        </h4>
        <div>
          <div className="flex gap-5 items-center mx-2 mr-4 rounded-lg p-4 mt-3 cursor-pointer hover:bg-[#635BFF] hover:duration-100 hover:scale-105 text-[#FFFFFF]">
            ...
            <p className="w-20 h-6 font-medium -mt-1 text-lg font-Inria hidden md:block">
              Overview
            </p>
          </div>
          <div className="flex gap-5 items-center mx-2 mr-4  rounded-lg p-4 mt-3 cursor-pointer hover:bg-[#635BFF] hover:duration-100 hover:scale-105 text-[#FFFFFF]">
      ...
      <div className="ml-3">
        ...
        <div>
          {adminNavigationMenus.map((item) => (
            <div key={item.id}>
              <button
                className="flex justify-between ml-3 w-[60px] md:w-[270px] items-center mx-2 rounded-lg p-4 mt-3 cursor-pointer
             hover:bg-[#635BFF] hover:duration-100 hover:scale-105 text-[#FFFFFF]"
                onClick={() => toggleVisibility(item.mainButton)}
              >
                <div className="flex gap-3 items-center">
                  <span className="md:ml-4 w-6 h-6 -mt-1.5">
                    {item.mainButtonIcons}
                  </span>
                  <p className="w-20 h-6 font-medium -mt-1 text-lg font-Inria text-start hidden md:block">
                    {item.mainButton}
                  </p>
                </div>
                <span
                  className={`w-4 h-4 -mt-3 hidden md:block 
                ${
                  visibleList === "Question"
                    ? "rotate-90 transition-all hover:duration-100 hover:scale-105"
                    : "rotate-0"
                }`}
                >
                ...
                <div className="md:ml-14 mt-1 flex items-center ">
                  ...
                  <div className="md:w-full list-group">
                    {item.subMainButtons.map((button) => (
                      <div
                        className="flex md:ml-3 md:mr-4 items-center rounded-lg p-1 md:p-3 md:mt-1 cursor-pointer
                    hover:bg-[#635BFF] hover:duration-100 hover:scale-105 text-[#FFFFFF]"
                      >
                        <p className="md:w-16 h-6 font-medium md:ml-2 text-[12px] mt-1 md:text-base whitespace-nowrap font-Inria">
                          {button}
                        </p>
                      </div>
                    ))}
